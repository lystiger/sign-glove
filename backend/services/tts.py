"""
    TTS Service
"""
import asyncio
import os
import logging
import hashlib
from concurrent.futures import ThreadPoolExecutor
from typing import Dict, Any, Optional

# TTS Libraries Import Check
try:
    import pyttsx3
    HAS_PYTTSX3 = True
except ImportError:
    HAS_PYTTSX3 = False
try:
    from gtts import gTTS
    HAS_GTTS = True
except ImportError:
    HAS_GTTS = False
try:
    import edge_tts
    HAS_EDGE_TTS = True
except ImportError:
    HAS_EDGE_TTS = False

# TTS Configuration
TTS_CONFIG = {
    "enabled": True,
    # microsoft edge tts, using edge right now cause it's the only one that supports Vietnamese
    "provider": "edge",  
    "voice": "vi-VN-HoaiMyNeural",
    "rate": 150,
    "volume": 0.8,
    "cache_enabled": True,
    "cache_dir": "tts_cache"
}

# Gesture mapping
GESTURE_MAPPING = {
    "Class 0": "",
    "Class 1": "", 
    "Class 2": "",
    "Class 3": "",
    "Class 4": "",
    "Class 5": "",
    "Class 6": "",
    "Class 7": "",
    "Class 8": "",
    "Class 9": "",
    "Class 10": "",
}

class TTSService:
    def __init__(self):
        self.executor = ThreadPoolExecutor(max_workers=2)
        self.cache = {}
        self.pyttsx3_engine = None
        
        # Đảm bảo thư mục cache tồn tại
        if TTS_CONFIG["cache_enabled"] and not os.path.exists(TTS_CONFIG["cache_dir"]):
            os.makedirs(TTS_CONFIG["cache_dir"])
        # Nếu thư viện pyttsx3 tồn tại, khởi tạo
        if HAS_PYTTSX3:
            self._init_pyttsx3()
    
    # Khởi tạo pyttsx3 engine bằng những config đã có
    def _init_pyttsx3(self):
        try:
            self.pyttsx3_engine = pyttsx3.init()
            self.pyttsx3_engine.setProperty('rate', TTS_CONFIG["rate"])
            self.pyttsx3_engine.setProperty('volume', TTS_CONFIG["volume"])
        except Exception as e:
            logging.error(f"Failed to initialize pyttsx3: {e}")
    
    def get_cache_path(self, text, voice):
        text_hash = hashlib.md5(f"{text}_{voice}".encode()).hexdigest()
        return os.path.join(TTS_CONFIG["cache_dir"], f"{text_hash}.mp3")

    async def speak(self, text: str, voice: str = None):
        """
        @effects: Speak the given text using the configured TTS provider.
        @param text: text cần đọc
        @param voice: tùy chỉnh
        """
        if not TTS_CONFIG["enabled"]:
            return {"status": "disabled"}
        
        provider = TTS_CONFIG["provider"]
        try:
            if provider == "edge" and HAS_EDGE_TTS:
                return await self._speak_edge(text, voice)
            elif provider == "gtts" and HAS_GTTS:
                return await self._speak_gtts(text)
            elif provider == "pyttsx3" and HAS_PYTTSX3:
                return await self._speak_pyttsx3(text)
            else:
                return {"status": "error", "message": "No TTS provider available"}
        except Exception as e:
            logging.error(f"TTS error: {e}")
            return {"status": "error", "message": str(e)}
    
    async def _speak_edge(self, text, voice: str = None):
        """
        @effects: Speak the given text using Microsoft Edge TTS.
        @param text: text cần đọc
        @param voice: tùy chỉnh giọng nói
        """
        voice = voice or TTS_CONFIG["voice"]
        cache_path = self.get_cache_path(text, voice)
        
        if TTS_CONFIG["cache_enabled"] and os.path.exists(cache_path):
            return {"status": "success", "provider": "edge", "audio_path": cache_path}
        
        # Edge TTS cần tạo một đối tượng Communicate để phát âm
        communicate = edge_tts.Communicate(text, voice)
        await communicate.save(cache_path)
        return {"status": "success", "provider": "edge", "audio_path": cache_path}
    
    async def _speak_gtts(self, text: str):
        """
        @effects: Speak the given text using Google Text-to-Speech (gTTS).
        @param text: text cần đọc
        """
        cache_path = self.get_cache_path(text, "vi")
        
        if TTS_CONFIG["cache_enabled"] and os.path.exists(cache_path):
            return {"status": "success", "provider": "gtts", "audio_path": cache_path}
        
        tts = gTTS(text=text, lang="vi")
        tts.save(cache_path)
        return {"status": "success", "provider": "gtts", "audio_path": cache_path}
    
    async def _speak_pyttsx3(self, text: str):
        """
        @effects: Speak the given text using pyttsx3.
        @param text: text cần đọc
        """
        if not self.pyttsx3_engine:
            return {"status": "error", "message": "pyttsx3 not available"}
        
        loop = asyncio.get_event_loop()
        success = await loop.run_in_executor(self.executor, self._pyttsx3_speak, text)
        return {"status": "success" if success else "error", "provider": "pyttsx3"}
    
    def _pyttsx3_speak(self, text):
        """
        @effects: Speak the given text using pyttsx3 in a separate thread.
        @param text: text cần đọc
        """
        try:
            self.pyttsx3_engine.say(text)
            self.pyttsx3_engine.runAndWait()
            return True
        except:
            return False
    
    def get_gesture_text(self, label):
        return GESTURE_MAPPING.get(label, label)
    
    def get_config(self):
        return {
            "config": TTS_CONFIG,
            "available_providers": {
                "pyttsx3": HAS_PYTTSX3,
                "gtts": HAS_GTTS,
                "edge_tts": HAS_EDGE_TTS
            },
            "gesture_mapping": GESTURE_MAPPING
        }
    
    def update_config(self, config: dict):
        global TTS_CONFIG
        TTS_CONFIG.update(config)
    
    def cleanup(self):
        self.executor.shutdown(wait=False)

# Global instance
tts_service = TTSService()